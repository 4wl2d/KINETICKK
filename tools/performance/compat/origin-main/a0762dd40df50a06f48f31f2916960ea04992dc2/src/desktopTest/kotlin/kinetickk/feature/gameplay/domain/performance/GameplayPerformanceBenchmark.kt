...
package kinetickk.feature.gameplay.domain.performance
...
import kinetickk.feature.gameplay.domain.reducer.GameReduction
...
import kinetickk.feature.gameplay.domain.simulation.addEnemyForTesting
import kinetickk.feature.gameplay.domain.simulation.copyForReduction
import kinetickk.feature.gameplay.domain.simulation.simulateStep
...
import kinetickk.feature.gameplay.domain.simulation.togglePause
...
fun main() {
    ...
    runBenchmarkSuite(
        identity = BenchmarkSuiteIdentity(
            ...
            adapter = "origin-main-a0762dd-compat",
            label = System.getProperty("kinetickk.benchmark.label", "origin/main"),
            revision = System.getProperty(
                "kinetickk.benchmark.revision",
                "a0762dd40df50a06f48f31f2916960ea04992dc2",
            ),
...
private class GameplayBenchmarkFixtures(private val seed: Int) {
    private val frame60 = GameplayAction.FrameElapsed(1f / 60f)
    private val frame100Millis = GameplayAction.FrameElapsed(0.1f)
    private val pointerMove = GameplayAction.PointerMoved(1_024f, 288f)
    private val viewportChange = GameplayAction.ViewportChanged(1_920f, 1_080f, 2f)
    ...
    fun scenarios(): List<BenchmarkScenario> = listOf(
        ...
        BenchmarkScenario(
            ...
            stateSignature(idle.copyForReduction())
        },
        BenchmarkScenario(
            name = "copy_capacity",
            category = "state",
            ...
            stateSignature(capacity.copyForReduction())
        },
        BenchmarkScenario(
        ...
        BenchmarkScenario(
            ...
            authoritySignature(idle, frame60)
        },
        BenchmarkScenario(
            ...
            authoritySignature(capacity, frame60)
        },
        BenchmarkScenario(
            ...
            authoritySignature(idle, pointerMove)
        },
        BenchmarkScenario(
            ...
            authoritySignature(idle, viewportChange)
        },
        BenchmarkScenario(
            ...
            authoritySignature(paused, frame60)
        },
        BenchmarkScenario(
            ...
            publishedPipelineSignature(idle, frame60)
        },
        BenchmarkScenario(
            ...
            publishedPipelineSignature(idle, pointerMove)
        },
        BenchmarkScenario(
            ...
            publishedPipelineSignature(paused, frame60)
        },
        BenchmarkScenario(
            name = "trace_2s_60hz",
            category = "trace",
    ...
    private fun outcomeFingerprint(name: String): Long = when (name) {
        ...
        "reducer_frame_60hz_idle" ->
            reductionOutcomeFingerprint(idle, frame60, includeRender = false)
        "reducer_frame_100ms_idle" ->
            reductionOutcomeFingerprint(idle, frame100Millis, includeRender = false)
        ...
        "nucleus_frame_60hz_idle", "published_frame_60hz_idle" ->
            reductionOutcomeFingerprint(idle, frame60, includeRender = true)
        "nucleus_frame_60hz_capacity" ->
            reductionOutcomeFingerprint(capacity, frame60, includeRender = true)
        "nucleus_pointer_move_idle", "published_pointer_move_idle" ->
            reductionOutcomeFingerprint(idle, pointerMove, includeRender = true)
        "nucleus_viewport_change_idle" ->
            reductionOutcomeFingerprint(idle, viewportChange, includeRender = true)
        "nucleus_frame_paused", "published_frame_paused" ->
            reductionOutcomeFingerprint(paused, frame60, includeRender = true)
    ...
    private fun reductionOutcomeFingerprint(
        model: MutableGameState,
        pulse: GameplayAction,
        includeRender: Boolean,
    ): Long {
        val reduction = acceptedReduction(reducer.reduce(EngineState(model), pulse))
        return canonicalDecisionOutcomeFingerprint(
            state = reduction.state.model,
            outputFingerprint = gameEffectFingerprint(reduction.effects),
            includeRender = includeRender,
        )
    }

    private fun reducerSignature(model: MutableGameState, pulse: GameplayAction): Long {
        val reduction = acceptedReduction(reducer.reduce(EngineState(model), pulse))
        return stateSignature(reduction.state.model) xor reduction.effects.size.toLong()
    }

    private fun authoritySignature(model: MutableGameState, pulse: GameplayAction): Long {
        val reduction = acceptedReduction(reducer.reduce(EngineState(model), pulse))
        val render = reduction.state.model.toRenderModel()
        val committed = GameDispatchResult.Committed(
            snapshot = GameSnapshot(revision = 1uL, renderModel = render),
            effects = reduction.effects,
        )
        var signature = stateSignature(reduction.state.model) xor committed.effects.size.toLong()
        signature = mix(signature, committed.snapshot.renderModel.enemies.size)
        signature = mix(signature, committed.snapshot.renderModel.projectiles.size)
        signature = mix(signature, committed.snapshot.renderModel.elapsed.toRawBits())
        return signature
    }
    ...
    private fun publishedPipelineSignature(
        ...
        val reduction = acceptedReduction(reducer.reduce(EngineState(model), pulse))
        // The origin/main engine publishes one fresh projection after a commit. Its pointer query
        // reads the revision-keyed cached snapshot, so it does not build another projection.
        val render = reduction.state.model.toRenderModel()
        var signature = stateSignature(reduction.state.model)
    ...
    private fun deterministicTraceFingerprint(traceSeed: Int): Long {
        var state = EngineState(newRunningState(traceSeed))
        repeat(TRACE_FRAMES) { frameIndex ->
            val x = 640f + ((frameIndex * 37) % 560)
            val y = 360f + ((frameIndex * 19) % 240) - 120f
            state = acceptedReduction(
                reducer.reduce(state, GameplayAction.PointerMoved(x, y)),
            ).state
            state = acceptedReduction(reducer.reduce(state, frame60)).state
            val render = state.model.toRenderModel()
            GameSnapshot(revision = (frameIndex + 1).toULong(), renderModel = render)
        }
        return canonicalStateFingerprint(state.model)
    }
}

private fun acceptedReduction(result: GameReductionResult): GameReduction =
    when (result) {
...
private fun gameEffectFingerprint(effects: Iterable<GameEffect>): Long {
    var signature = SEMANTIC_OUTPUT_SEED
    effects.forEach { effect ->
        signature = when (effect) {
            is GameEffect.EmitVisualFx ->
                appendSemanticBatch(signature, SEMANTIC_VISUAL_FX, effect.cues)
            is GameEffect.AdvanceAudio -> appendSemanticAudio(
                signature,
                effect.realDeltaSeconds,
                effect.cues,
            )
            is GameEffect.PublishProgress -> mix(signature, SEMANTIC_PROFILE_UPDATE)
