...
import void.kinetic.audio.SoundCue
...
data class DamageNumber(
    ...
    private val compactAmount = formatDamageNumber(amount, DamageNumberFormat.COMPACT)
    private val fullAmount = formatDamageNumber(amount, DamageNumberFormat.FULL)
...
data class WeaponArc(
    val fromX: Float,
    val fromY: Float,
    val toX: Float,
    val toY: Float,
    var life: Float = 0.14f,
)
...
class GameEngine(
    ...
    private val gameplayRandom = Random(seed)
    private val fxRandom = Random(seed xor 0x5EED_C0DE)
    private val persistenceEnabled = initialMatter == null
    private val storedProgress = if (persistenceEnabled) ProgressCodec.decode(loadProgress()) else null
    private var activeRebirthProfile = RebirthProgression.profile(
        storedProgress?.rebirthLevel ?: initialRebirthLevel,
    )
    ...
    private val unlockedWeaponSet = mutableSetOf<WeaponId>().apply {
        val stored = storedProgress?.unlockedWeaponIndices.orEmpty()
    ...
    private val metaRanks = IntArray(MetaUpgradeId.entries.size) { index ->
        ...
        storedProgress?.metaLevels?.getOrNull(index)?.coerceIn(0, definition.maxRanks) ?: 0
    }
    private val discoveredItemIds = storedProgress?.discoveredItemIds
    ...
    var screen = UiScreen.GAME
        private set
    var settings = storedProgress?.settings ?: GameSettings()
        private set
    var rebirthLevel = activeRebirthProfile.tier
        private set
    var highestClearedRebirth = storedProgress?.highestClearedRebirth
    ...
    var runMatter = 0L
        private set
    var totalMatter = initialMatter?.toLong() ?: storedProgress?.matter ?: loadMatter().toLong()
        private set
    var lifetimeMatter = initialMatter?.toLong() ?: storedProgress?.lifetimeMatter ?: totalMatter
        private set
    var lastImpact = 0f
        private set
    ...
    var weapon = WeaponId.entries.getOrElse(storedProgress?.selectedWeaponIndex ?: 0) { WeaponId.FLUX_WAKE }
        private set
    ...
    val motionEchoes = mutableListOf<MotionEcho>()
    ...
    val weaponArcs = mutableListOf<WeaponArc>()
    ...
    init {
        ...
        val storedShape = CoreShape.entries.getOrElse(storedProgress?.coreShapeIndex ?: 0) { CoreShape.ORB }
    ...
    private fun sampleMotionEcho(delta: Float) {
        val intensity = clamp((speed - 260f) / 1_500f, 0f, 1f)
        if (intensity <= 0f && dashPhaseTime <= 0f) {
            motionEchoClock = 0f
            return
        }
        motionEchoClock -= delta
        if (motionEchoClock > 0f) return
        val dashIntensity = if (dashPhaseTime > 0f) 1f else intensity
        motionEchoClock = if (dashPhaseTime > 0f) 0.018f else 0.075f - intensity * 0.04f
        val maxLife = 0.2f + dashIntensity * 0.16f
        motionEchoes += MotionEcho(previousCoreX, previousCoreY, maxLife, maxLife, dashIntensity)
        while (motionEchoes.size > 36) motionEchoes.removeAt(0)
    }
    ...
    private fun sampleFluxTrail() {
        ...
        while (trail.size > 110) trail.removeAt(0)
        trailLastX = coreX
        trailLastY = coreY
    ...
    private fun fireArcCoil(baseDamage: Float) {
        ...
        targets.forEachIndexed { index, enemy ->
            ...
            weaponArcs += WeaponArc(fromX, fromY, enemy.x, enemy.y)
            fromX = enemy.x
            fromY = enemy.y
        }
        val agonyRank = relicRank(RelicId.AGONY_SCEPTER)
    ...
    private fun fireSingularitySpear(damage: Float) {
        ...
        if (agonyRank > 0) {
            ...
            weaponArcs += WeaponArc(coreX, coreY, crossEndX, crossEndY, 0.18f)
    ...
    private fun updateProjectiles(delta: Float) {
        ...
        while (projectiles.size > 650) projectiles.removeAt(0)
    }
    ...
    private fun updatePickups(delta: Float) {
        ...
        while (pickups.size > 420) pickups.removeAt(0)
    }
    ...
    private fun updateEffects(delta: Float) {
        particles.forEach {
            it.x += it.vx * delta
            it.y += it.vy * delta
            it.vx *= exp(-2.2f * delta)
            it.vy *= exp(-2.2f * delta)
            it.life -= delta
        }
    ...
    private fun spawnWave(delta: Float) {
        ...
        val maxEnemies = rebirthProfile.enemyCap(baseMaxEnemies)
    ...
    private fun redirectPrismRelay(projectile: Projectile) {
        ...
        weaponArcs += WeaponArc(projectile.x, projectile.y, target.x, target.y, 0.08f)
    }
        ...
        while (iterator.hasNext()) {
            ...
            if (segmentCircleIntersects(
                ...
                when (pickup.type) {
                    ...
                    PickupType.KEY -> {
                        keys++
                        message = "VOID KEY ACQUIRED"
                        messageTime = 1.4f
                    }
                    PickupType.REPAIR -> hp = min(maxHp, hp + 22f)
                    PickupType.RELIC -> {
                        pendingRelicChoices++
                        message = "VOID RELIC RESONANCE"
                        messageTime = 1.4f
                    }
                }
    ...
    private fun damageEnemy(
        ...
        if (settings.damageNumbers && amount >= 5f && damageNumbers.size < 140) {
    ...
    private fun onQualifiedWeaponHit(enemy: Enemy, result: DamageResult, sourceWeapon: WeaponId) {
        ...
        if (echoRank > 0 && delayedRelicHits.size < 256) {
    ...
    private fun addRelicArc(fromX: Float, fromY: Float, toX: Float, toY: Float) {
        weaponArcs += WeaponArc(fromX, fromY, toX, toY, 0.12f)
        while (weaponArcs.size > 128) weaponArcs.removeAt(0)
    }
    ...
    private fun onEnemyKilled(enemy: Enemy) {
        ...
        repeat(dataCount) {
            ...
            pickups += Pickup(PickupType.DATA, enemy.x, enemy.y, cos(angle) * pickupSpeed, sin(angle) * pickupSpeed)
        }
        if (enemy.type == EnemyType.ELITE) {
            pickups += Pickup(PickupType.KEY, enemy.x, enemy.y)
            pickups += Pickup(PickupType.RELIC, enemy.x + 15f, enemy.y - 8f)
        }
        if (enemy.type != EnemyType.ARCHITECT && gameplayRandom.nextFloat() < 0.035f + luck * 0.02f) {
            pickups += Pickup(PickupType.REPAIR, enemy.x, enemy.y)
        }
                    ...
                    ChoiceOption(
                        ...
                        description = if (currentRank > 0) {
                            if (currentRank < RelicCatalog.MAX_RANK) {
                                ...
                                "This resonance is already rank ${RelicCatalog.MAX_RANK}; selecting it salvages Void Matter."
                            }
                        } else {
                            relic.description
                        },
    ...
    private fun persist() {
        ...
        saveProgress(ProgressCodec.encode(progress))
        saveMatter(totalMatter.coerceAtMost(Int.MAX_VALUE.toLong()).toInt())
    }

    fun drainSoundCues(): List<SoundCue> {
    ...
    private fun fireSpread(x: Float, y: Float, angle: Float, count: Int, spacing: Float, projectileSpeed: Float) {
        ...
        repeat(count) { index ->
            ...
            projectiles += Projectile(x, y, cos(shotAngle) * scaledSpeed, sin(shotAngle) * scaledSpeed, 5f, 6f)
        }
    }
    ...
    private fun fireRadial(x: Float, y: Float, count: Int, projectileSpeed: Float, rotation: Float) {
        ...
        repeat(count) { index ->
            ...
            projectiles += Projectile(x, y, cos(angle) * scaledSpeed, sin(angle) * scaledSpeed, 5f, 8f)
        }
    }

    private fun burst(x: Float, y: Float, requestedCount: Int, colorIndex: Int) {
        val multiplier = when (settings.particleDensity) {
            ParticleDensity.LOW -> 0.45f
            ParticleDensity.NORMAL -> 1f
            ParticleDensity.HIGH -> 1.4f
        }
        val count = (requestedCount * multiplier).toInt().coerceAtLeast(1)
        repeat(count) {
            if (particles.size >= 700) return@repeat
            val angle = fxRandom.nextFloat() * TAU
            val particleSpeed = 35f + fxRandom.nextFloat() * 185f
            val life = 0.25f + fxRandom.nextFloat() * 0.55f
            particles += Particle(x, y, cos(angle) * particleSpeed, sin(angle) * particleSpeed, life, life, colorIndex, 1.5f + fxRandom.nextFloat() * 3.5f)
        }
    }
    ...
    private fun directionalBurst(
        ...
        repeat(count) {
            if (particles.size >= 700) return@repeat
            val angle = baseAngle + (fxRandom.nextFloat() - 0.5f) * 1.15f
            val particleSpeed = 90f + fxRandom.nextFloat() * 310f
            val life = 0.22f + fxRandom.nextFloat() * 0.42f
            particles += Particle(
                x,
                y,
                ...
                life,
                colorIndex,
                1.8f + fxRandom.nextFloat() * 4.2f,
            )
        }
    }

    private fun shockwave(x: Float, y: Float, life: Float, maxRadius: Float, colorIndex: Int) {
        shockwaves += Shockwave(x, y, life, life, maxRadius, colorIndex)
        while (shockwaves.size > 48) shockwaves.removeAt(0)
    }
    ...
    private fun rebaseWorldIfNeeded() {
        ...
        particles.forEach { it.x -= shiftX; it.y -= shiftY }
        damageNumbers.forEach { it.x -= shiftX; it.y -= shiftY }
