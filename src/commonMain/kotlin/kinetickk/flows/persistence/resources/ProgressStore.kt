...
sealed interface ProgressLoadResult {
    data class Loaded(val progress: PersistedProgress) : ProgressLoadResult
...
interface ProgressStore {
    ...
    fun persist(progress: PersistedProgress): ProgressPersistResult
}
...
private fun decodeLegacyMatter(payload: String): ProgressLoadResult = when (payload.utf8Validation()) {
    ...
    Utf8Validation.Accepted -> payload.toIntOrNull()
        ?.coerceAtLeast(0)
        ?.toLong()
        ?.let { ProgressLoadResult.Loaded(PersistedProgress(matter = it, lifetimeMatter = it)) }
