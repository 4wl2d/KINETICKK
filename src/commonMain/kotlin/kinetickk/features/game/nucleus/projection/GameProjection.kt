...
package kinetickk.features.game.nucleus.projection
...
import kinetickk.features.game.nucleus.EquippedRelic
...
import kinetickk.features.game.nucleus.distanceSquared
import kinetickk.features.game.nucleus.formatDamageNumber
import kinetickk.features.game.nucleus.length
...
/**
 * Immutable render/query payload. Canonical read-result metadata is supplied
 * by the Ball read boundary; no mutable Nucleus container crosses this type.
 */
class GameProjection internal constructor(
    ...
    fun isHudControlPosition(x: Float, y: Float): Boolean {
        if (screen != UiScreen.GAME || phase != GamePhase.RUNNING) return false
        val dashX = screenWidth - 82f * uiScale
        val dashY = screenHeight - 88f * uiScale
        val brakeX = screenWidth - 190f * uiScale
        val brakeY = screenHeight - 67f * uiScale
        return distanceSquared(x, y, dashX, dashY) < 48f * uiScale * 48f * uiScale ||
            distanceSquared(x, y, brakeX, brakeY) < 38f * uiScale * 38f * uiScale
    }
