...
import kinetickk.features.game.nucleus.ChoiceOption
import kinetickk.features.game.nucleus.ChoiceType
import kinetickk.features.game.nucleus.CoreShape
import kinetickk.features.game.nucleus.DAMAGE_NUMBER_DEVASTATING_MULTIPLIER
import kinetickk.features.game.nucleus.DAMAGE_NUMBER_POWERFUL_MULTIPLIER
import kinetickk.features.game.nucleus.DamageNumberTier
...
import kinetickk.features.game.nucleus.ItemDefinition
import kinetickk.features.game.nucleus.ItemRarity
import kinetickk.features.game.nucleus.MetaUpgradeCatalog
import kinetickk.features.game.nucleus.MetaUpgradeDefinition
...
import kinetickk.features.game.nucleus.RelicCatalog
import kinetickk.features.game.nucleus.RelicChoiceAction
import kinetickk.features.game.nucleus.RelicId
import kinetickk.features.game.nucleus.SettingsRow
import kinetickk.features.game.nucleus.TAU
import kinetickk.features.game.nucleus.TotemAction
import kinetickk.features.game.nucleus.UiScreen
import kinetickk.features.game.nucleus.WeaponCatalog
import kinetickk.features.game.nucleus.WeaponDefinition
import kinetickk.features.game.nucleus.WeaponId
import kinetickk.features.game.nucleus.WeaponMastery
import kinetickk.features.game.nucleus.WeaponNodeType
import kinetickk.features.game.nucleus.abbreviateNumber
import kinetickk.features.game.nucleus.clamp
import kinetickk.features.game.nucleus.damageNumberTier
import kinetickk.features.game.nucleus.formatRunTime
import kinetickk.features.game.nucleus.settingsRowsPerPage
...
private val SpaceBlack = Color(0xFF050610)
private val OverlayPanel = Color(0xE60B0D1D)
...
private val VelocityNames = listOf("DRIFT", "SURGE", "HYPER", "OVERDRIVE", "TRANSCENDENT")
...
internal fun DrawScope.drawKinetickk(
...
private fun DrawScope.drawBackdrop(engine: GameProjection, shakeX: Float, shakeY: Float, renderTime: Float) {
...
private fun DrawScope.drawSpeedField(engine: GameProjection, shakeX: Float, shakeY: Float) {
...
private fun DrawScope.drawMotionEchoes(
    engine: GameProjection,
    visualFx: VisualFxProjection,
    shakeX: Float,
    shakeY: Float,
    ...
    visualFx.motionEchoes.forEach { echo ->
        ...
        val radius = GameProjection.CORE_RADIUS + (1f - life) * 9f
...
private fun DrawScope.drawTrail(engine: GameProjection, shakeX: Float, shakeY: Float) {
...
private fun DrawScope.drawScreenFx(engine: GameProjection, renderTime: Float) {
...
private fun DrawScope.drawWeapon(engine: GameProjection, core: Offset, shakeX: Float, shakeY: Float) {
...
private fun DrawScope.drawWeaponNodes(engine: GameProjection, shakeX: Float, shakeY: Float) {
...
private fun DrawScope.drawWeaponArcs(
    engine: GameProjection,
    visualFx: VisualFxProjection,
    shakeX: Float,
    shakeY: Float,
...
private fun DrawScope.drawCore(engine: GameProjection, center: Offset) {
    ...
    when (engine.coreShape) {
        CoreShape.ORB -> {
            drawCircle(main, GameProjection.CORE_RADIUS, center)
...
private fun DrawScope.drawEnemy(engine: GameProjection, enemy: EnemyProjection, shakeX: Float, shakeY: Float) {
...
private fun DrawScope.drawProjectiles(engine: GameProjection, shakeX: Float, shakeY: Float) {
...
private fun DrawScope.drawPickups(engine: GameProjection, shakeX: Float, shakeY: Float) {
...
private fun DrawScope.drawTotem(engine: GameProjection, shakeX: Float, shakeY: Float, textMeasurer: TextMeasurer) {
...
private fun DrawScope.drawHud(engine: GameProjection, textMeasurer: TextMeasurer) {
    ...
    drawBar(contentX, panelY + d(111f), barWidth, d(6f), engine.heat / GameProjection.MAX_HEAT, if (engine.overheated) Red else Orange, DarkLine)
...
private fun DrawScope.drawRelicMatrix(engine: GameProjection, textMeasurer: TextMeasurer, narrow: Boolean, right: Float) {
...
private fun DrawScope.drawControls(engine: GameProjection, textMeasurer: TextMeasurer) {
    ...
    drawArc(
        ...
        -90f,
        360f * clamp(1f - engine.heat / GameProjection.MAX_HEAT, 0f, 1f),
        false,
...
private fun DrawScope.drawMenu(engine: GameProjection, textMeasurer: TextMeasurer, renderTime: Float) {
    val narrow = size.width / density < 700f
...
private fun DrawScope.drawChoice(engine: GameProjection, textMeasurer: TextMeasurer, renderTime: Float) {
...
private fun DrawScope.drawChoiceCard(engine: GameProjection, textMeasurer: TextMeasurer, choice: ChoiceOption, index: Int, x: Float, y: Float, width: Float, height: Float, renderTime: Float) {
...
private fun DrawScope.drawEnd(engine: GameProjection, textMeasurer: TextMeasurer, victory: Boolean) {
...
private fun DrawScope.drawSettings(engine: GameProjection, textMeasurer: TextMeasurer) {
...
private fun DrawScope.settingValue(engine: GameProjection, row: SettingsRow): String = when (row) {
...
private fun DrawScope.drawLab(engine: GameProjection, textMeasurer: TextMeasurer) {
    drawRect(Color(0xD9050610))
...
private fun DrawScope.drawMetaCard(engine: GameProjection, textMeasurer: TextMeasurer, definition: MetaUpgradeDefinition, x: Float, y: Float, width: Float, height: Float) {
...
private fun DrawScope.drawRebirth(engine: GameProjection, textMeasurer: TextMeasurer) {
    drawRect(Color(0xD9050610))
    val bounds = overlayBounds()
...
private fun DrawScope.drawArmory(engine: GameProjection, textMeasurer: TextMeasurer, renderTime: Float) {
    drawRect(Color(0xD9050610))
    val bounds = overlayBounds()
...
private fun DrawScope.drawWeaponCard(engine: GameProjection, textMeasurer: TextMeasurer, definition: WeaponDefinition, x: Float, y: Float, width: Float, height: Float, renderTime: Float) {
...
private fun DrawScope.drawCodex(engine: GameProjection, textMeasurer: TextMeasurer) {
    drawRect(Color(0xD9050610))
    val bounds = overlayBounds()
...
private fun DrawScope.drawCodexItem(engine: GameProjection, textMeasurer: TextMeasurer, item: ItemDefinition, x: Float, y: Float, width: Float, height: Float) {
...
private fun DrawScope.world(engine: GameProjection, x: Float, y: Float, shakeX: Float, shakeY: Float): Offset =
