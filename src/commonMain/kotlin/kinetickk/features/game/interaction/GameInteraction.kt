...
import kinetickk.application.assembly.GameAssembly
...
@Composable
fun KinetickkApp() {
    val assembly = remember { GameAssembly.create() }
    val focusRequester = remember { FocusRequester() }
    val composeTextMeasurer = rememberTextMeasurer(cacheSize = 64)
    val density = LocalDensity.current.density
    val interactionValidator = remember(assembly) { GameInteractionValidator() }
    var projectionValue by remember(assembly) { mutableStateOf(assembly.readGameProjection()) }
    var visualFxProjectionValue by remember(assembly) {
        mutableStateOf(assembly.visualFxSnapshot())
    }
    ...
    fun dispatch(intent: GameIntent) {
        when (val result = assembly.dispatch(intent)) {
            is GameDispatchResult.Committed -> {
                ...
                visualFxProjectionValue = assembly.visualFxSnapshot()
            }
            is GameDispatchResult.DecisionRejected,
            is GameDispatchResult.AdmissionRejected
            -> Unit
        }
    }
    ...
    DisposableEffect(assembly) {
        onDispose { assembly.close() }
    }

    LaunchedEffect(assembly) {
                ...
                when (event.key) {
                    ...
                    Key.R -> {
                        if (
                            ...
                            assembly.readGameProjection().phase in listOf(GamePhase.GAME_OVER, GamePhase.VICTORY)
                        ) {
            ...
            .pointerInput(assembly) {
                awaitPointerEventScope {
                    ...
                    while (true) {
                        ...
                        val currentProjection = assembly.readGameProjection()
                        ...
                        if (pressed && !wasPressedValue && validatedMove != null) {
                            dispatch(GameIntent.UserGestureObserved)
                            dispatchValidated(
                                interactionValidator.pointerPressed(
                                    rawXpx = validatedMove.x,
                                    rawYpx = validatedMove.y,
                                ),
                            )
                        }
                        if (!pressed && wasPressedValue) {
                            dispatch(GameIntent.PointerReleased)
