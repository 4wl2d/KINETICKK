...
import kinetickk.feature.game.domain.model.ItemRarity
...
internal fun DrawScope.drawOverlayFrame(bounds: Rect, accent: Color) {
...
internal fun DrawScope.drawFooterBack(textMeasurer: TextMeasurer, bounds: Rect, accent: Color) {
...
internal fun DrawScope.drawLabFooter(textMeasurer: TextMeasurer, bounds: Rect, accent: Color) {
    val top = bounds.bottom - d(55f)
...
internal fun DrawScope.drawPagedFooter(textMeasurer: TextMeasurer, bounds: Rect, page: Int, maxPage: Int, accent: Color) {
    val top = bounds.bottom - d(55f)
...
internal fun DrawScope.overlayBounds(maxWidth: Float = 900f, maxHeight: Float = 650f): Rect {
...
internal fun DrawScope.drawWeaponGlyph(id: WeaponId, center: Offset, radius: Float, time: Float, color: Color) {
    when (id) {
        WeaponId.FLUX_WAKE -> {
...
internal fun weaponColor(id: WeaponId): Color = WeaponColors[id.ordinal.coerceIn(WeaponColors.indices)]

internal fun rarityColor(rarity: ItemRarity): Color = RarityColors[(rarity.rank - 1).coerceIn(RarityColors.indices)]
...
internal fun polar(center: Offset, radius: Float, angle: Float): Offset =
...
internal fun DrawScope.drawBar(x: Float, y: Float, width: Float, height: Float, progress: Float, foreground: Color, background: Color) {
...
internal fun DrawScope.drawPolygon(center: Offset, radius: Float, sides: Int, rotation: Float, color: Color, style: androidx.compose.ui.graphics.drawscope.DrawStyle) {
    val path = Path()
    repeat(sides) { index ->
...
internal fun DrawScope.world(engine: GameProjection, x: Float, y: Float, shakeX: Float, shakeY: Float): Offset =
    Offset(size.width * 0.5f + x - engine.cameraX + shakeX, size.height * 0.5f + y - engine.cameraY + shakeY)

internal fun DrawScope.isOnScreen(point: Offset, margin: Float = 0f): Boolean =
    point.x >= -margin && point.y >= -margin && point.x <= size.width + margin && point.y <= size.height + margin

internal fun DrawScope.d(value: Float): Float = value * density

internal fun positiveModulo(value: Float, modulus: Float): Float = ((value % modulus) + modulus) % modulus
