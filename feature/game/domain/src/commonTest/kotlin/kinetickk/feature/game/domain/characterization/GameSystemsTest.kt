...
class GameSystemsTest {
    ...
    fun eachMetaPurchaseDeductsLongMatterAndAddsExactlyOneRank() {
        val engine = GameScenario(seed = 30, initialMatter = Int.MAX_VALUE)
    ...
    fun permanentWeaponPurchaseUnlocksEquipsAndDoesNotChargeTwice() {
        val engine = GameScenario(seed = 31, initialMatter = 10_000)
        ...
        val poorEngine = GameScenario(seed = 32, initialMatter = 0)
    ...
    fun prismRelayRefractsFromItsFirstTargetIntoAnotherEnemy() {
        ...
        repeat(50) { engine.update(GameScenario.FIXED_STEP) }
    ...
    @Test
    fun dashRequestPreservesWorldAim() {
        ...
        engine.update(GameScenario.FIXED_STEP)
        assertTrue(engine.velocityX < -500f, "Dash request changed the current world aim")
    }
    ...
    fun brakeSourcesDoNotCancelEachOther() {
        ...
        engine.setTouchBrake(true)
        engine.setTouchBrake(false)
    ...
    fun continuousDamageDoesNotHoldFullWhiteFlash() {
        ...
        repeat(24) { engine.update(GameScenario.FIXED_STEP) }
    ...
    fun impactFeedbackExpires() {
        ...
        engine.enemies.clear()
        repeat(100) { engine.update(GameScenario.FIXED_STEP) }
    ...
    private fun exerciseWeapon(id: WeaponId): WeaponRuntimeSignature {
        ...
        val visualFxCues = engine.takeVisualFxCues()
        if (id == WeaponId.ARC_COIL) {
            assertTrue(visualFxCues.any { it is VisualFxCue.WeaponArcAdded })
        }
        ...
        return WeaponRuntimeSignature(
            ...
            arcCount = visualFxCues.count { it is VisualFxCue.WeaponArcAdded },
