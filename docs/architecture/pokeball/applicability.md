...
| cross-authority reads | Session, Gameplay, and Profile consume target-owned snapshots | target-owned query/result pairs, explicit freshness, Assembly bindings, non-atomic multi-read disclosure |
| same-stack commands and results | Session coordinates Profile/Gameplay; Gameplay reports progress to Profile | target-owned mappings, `CommandRef`, accepted-frame correlation, carrier/result exclusivity, depth reservation, completion deque |
...
| detached asynchronous semantic delivery | no actor/mailbox/coroutine queue/event bus carries a Pulse, command, result, output, or Fact; all semantic commands complete through the bounded same-stack deque. Resource-internal provider execution, including Desktop tone playback, is outside this absence scope and may not acquire business meaning |
| automatic delivery retry, root idempotency, or cancellation protocol | no accepted output is automatically redelivered; user `RetryLegacyPurge` is a fresh explicit Pulse, not delivery retry; no cancellable operation exists |
